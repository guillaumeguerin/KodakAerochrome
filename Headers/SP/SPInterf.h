...
typedef struct SPInterfaceSuite {
	...
	/** Starts up the plug-in in a plug-in list that exports a given suite.
		...
		*/
	SPAPI SPErr (*StartupExport)( SPPluginListRef pluginList, const char *name, int version,
				int *started );
...
/** Internal */
SPAPI SPErr SPStartupExport( SPPluginListRef pluginList, const char *name, int version,
			int *started );
