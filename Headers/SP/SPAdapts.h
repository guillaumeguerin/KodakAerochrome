...
typedef struct SPAdaptersSuite {
	...
	SPAPI SPErr (*AddAdapter)( SPAdapterListRef adapterList, struct SPPlugin *host, const char *name,
				int version, SPAdapterRef *adapter );
	...
	/** Retrieves the version of an adapter.
		...
		*/
	SPAPI SPErr (*GetAdapterVersion)( SPAdapterRef adapter, int *version );
...
/** Internal */
SPAPI SPErr SPGetAdapterVersion( SPAdapterRef adapter, int *version );
